# QR Code URL Reference

This document lists the stable GitHub URLs to encode in the kit QR codes.
These URLs target the `main` branch and will not change after initial publish.
...
- All URLs use the `main` branch — do not use commit SHAs in QR codes.
- Repository is public; no login required to access any of these URLs.
- URLs resolve to the folder view showing both the `.ino` file and `README.md`.
- Test each URL in a browser before generating final QR code artwork.
